...
export default function ArtistProfile({ navigate, artist, userRole = 'public', returnTo }: Props) {
  ...
  const handleUploadAvatar = async () => {
    ...
    try {
      ...
      setLastUpdate(Date.now()); // Force image refresh
    } catch (error: any) {
  ...
  // Determine which profile data to use (Context for own, Fetched for others, or fallback to nav prop)
  ...
  // Avatar Logic: DB > Nav Prop > Default
  const dbAvatar = effectiveProfile?.avatar_url;
  const navAvatar = artist?.avatar_url ?? artist?.profile_image ?? artist?.profile_image_url;
        ...
        <View style={styles.coverWrap}>
          ...
          >
            ...
            {isOwnProfile && !uploading && (
              <View style={styles.editBadge}><Camera size={14} color="#fff" /></View>
            )}
          </Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  shareBtn: {},
  profileImgContainer: { position: 'absolute', bottom: -64, left: 24, width: 128, height: 128, borderRadius: 64, borderWidth: 4, borderColor: '#030712', backgroundColor: '#1f2937' },
  profileImg: { width: '100%', height: '100%', borderRadius: 64 },
  loadingOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: 64, justifyContent: 'center', alignItems: 'center' },
  editBadge: { position: 'absolute', bottom: 0, right: 0, backgroundColor: '#a855f7', width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center', borderWidth: 2, borderColor: '#030712' },
